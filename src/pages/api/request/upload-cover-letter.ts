...
const uploadDir = path.join(process.cwd(), 'public', 'media', 'domain-request', 'uploads', 'cover-letter');
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  form.parse(req, async (err: any, fields: Fields, files: Files) => {
    ...
    try {
      ...
      const newFilePath = path.join('public', 'media', 'domain-request', 'uploads', 'cover-letter', fileName);

      fs.renameSync(file.filepath, path.join(process.cwd(), newFilePath));

      await prisma.request.update({
        where: { request_token: token },
        data: { uploaded_cover_letter_path: newFilePath },
      });

      res.status(200).json({ success: true, msg: 'Cover letter uploaded successfully' });
    } catch (error) {
      console.error('Error updating request with cover letter path:', error);
      res.status(400).json({ success: false, msg: 'Error updating request with cover letter path' });
    }
  });
}
